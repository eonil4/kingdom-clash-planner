...
export default function FormationHeader({ onFormationsClick }: FormationHeaderProps) {
  const currentFormation = useAppSelector((state) => state.formation.currentFormation);

  return (
    <Box
      component="header"
      className="w-full p-4 bg-gradient-to-b from-gray-700 to-gray-800 flex items-center justify-between"
      style={{
    ...
    >
      ...
      <Box className="flex flex-col items-center">
        <IconButton
          onClick={onFormationsClick}
          className="text-yellow-500 hover:bg-yellow-500 hover:bg-opacity-20"
          aria-label="View formations list"
        >
          <ViewListIcon className="text-3xl" />
        </IconButton>
        <Typography variant="caption" className="text-yellow-500 text-xs">
          Formations
        </Typography>
      </Box>
    </Box>
