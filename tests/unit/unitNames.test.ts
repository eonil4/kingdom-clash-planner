...
describe('unitNames', () => {
  ...
  describe('getUnitDataByName', () => {
    ...
    it('should handle case-sensitive unit names', () => {
      const result = getUnitDataByName('archers'); // lowercase
      // The function should match exact name, so lowercase might not match
      // This depends on implementation - if it's case-sensitive, it should return undefined
      expect(result).toBeUndefined();
    });
  });
  ...
  describe('getUnitDataByIndex', () => {
    ...
    it('should return unit data for all valid indices', () => {
      // Get all unique indices from UnitDataMap
